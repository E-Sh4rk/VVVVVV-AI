...
class ReplayMemory():
  ...
  def append(self, state, action, reward, terminal):
    state = state[-NB_CHANNELS:].mul(255).to(dtype=torch.uint8, device=torch.device('cpu'))  # Only store last frame and discretise to save memory
  ...
  def _get_sample_from_segment(self, segment, i):
    ...
    state = torch.stack(flatten([trans.state for trans in transition[:self.history]])).to(device=self.device).to(dtype=torch.float32).div_(255)
    next_state = torch.stack(flatten([trans.state for trans in transition[self.n:self.n + self.history]])).to(device=self.device).to(dtype=torch.float32).div_(255)
  ...
  def __next__(self):
    ...
    for t in reversed(range(self.history - 1)):
      if prev_timestep == 0:
        ...
        prev_timestep -= 1
    state = torch.stack(flatten(state_stack), 0).to(dtype=torch.float32, device=self.device).div_(255)  # Agent will turn into batch
    self.current_idx += 1
    return state
